  ...
  <div class="panel admin-panel">
    ...
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        <li>
          <input type="text" placeholder="请输入搜索关键字" name="stu_id"  id="stu_id" class="input" style="width:250px; line-height:17px;display:inline-block" />
           <input type="button" class="button bg-blue margin-left" id="add" value="+ 添加学生"  >
...
$(function(){
	...
	$("#delete").on("click",function(){
		...
		layer.confirm('您是是否要删除该用户信息？', {
			  btn: ['是','否'] //按钮
			}, function(){
				 $.ajax({
				       type:"POST",
				       async:false,
				       url: "stu/deleteStuById",
				       data:{account:stu_id
				       },
				       dataType:"json",
		       ...
		       success:function(data){
		    	   ...
		    	   for(var i=0;i<data.list.length;i++)
		   	    	 {  
		   	    		...
		   	    		$("#"+i).append('<td>'+data.list[i].user.userId+'</td>');
		   	    		$("#"+i).append('<td>'+data.list[i].studentId+'</td>');
		   	    		$("#"+i).append('<td>'+data.list[i].studentName+'</td>');
		   	    		$("#"+i).append('<td>'+data.list[i].user.userLimit+'</td></tr>');
		   	    	 }
